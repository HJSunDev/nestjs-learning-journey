...
└── createReactAgent（官方预构建 ReAct Agent，仅作参考工具函数保留）
```
 ...
 * 1. new StateGraph(MessagesAnnotation)
 * 2. addNode("agent", model.bindTools(tools).invoke)
 * 3. addNode("tools", new ToolNode(tools))
 * 4. START → agent → conditional → tools/END
 * 5. tools → agent
 * 6. compile()
 *
...
@Post('react/chat')
async reactChat(@Body() dto: ReactChatRequestDto) {
  return this.reactService.invoke({ ...dto });
}
...
| `src/ai/agents/single/react-agent/react-agent.prebuilt.ts` | createReactAgent 参考工具函数（不接入服务） |
...
│   │   ├── react-agent.prebuilt.ts   # createReactAgent 参考工具函数
│   │   └── index.ts
│   └── index.ts
├── multi/
│   └── index.ts
